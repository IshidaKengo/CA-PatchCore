...
def evaluation(args):
    ###Preparation####
    save_path = os.path.join(args.evaluation_path, f'num_clusters={args.num_cluster}')
    os.makedirs(save_path, exist_ok=True)
    
    load_path = os.path.join(args.output_path, f'num_clusters={args.num_cluster}', 'results')
    ...
    test_datasets = LoadDataset(args.dataset_path,  transform = [], phase='test')
    ...
    for pred, type in zip(scores_all,  test_datasets.types):
        if type == '0_good':
            ...
            scores_scratch.append(pred)
        elif type == '2_paint':
            scores_paint.append(pred)
        elif type == '3_over-coupling':
            scores_overcoupling.append(pred)
        else:
            scores_lacking.append(pred)
    
    scores_anomaly = scores_scratch + scores_paint + scores_overcoupling + scores_lacking
    
    ###Caliculate AUROC###
    auc_scratch = caliculate_AUROC(scores_good, scores_scratch)
    auc_paint = caliculate_AUROC(scores_good, scores_paint)
    auc_overcoupling = caliculate_AUROC(scores_good, scores_overcoupling)
    auc_lacking = caliculate_AUROC(scores_good, scores_lacking)
    auc_all = caliculate_AUROC(scores_good, scores_anomaly)
    print(f'AUC(whole dataset)  {auc_all:.3f}')
    print(f'AUC(anomaly categories)  scratch:{auc_scratch:.3f}   paint:{auc_paint:.3f}   over-coupling:{auc_overcoupling:.3f}   lacking:{auc_lacking:.3f}')
    
    with open(os.path.join(save_path, r'AUROC.csv'), 'a') as f:
        writer = csv.writer(f)
        writer.writerow(['All', 'scratch', 'paint', 'over-coupling', 'lacking'])    
        writer.writerow([auc_all, auc_scratch, auc_paint, auc_overcoupling, auc_lacking])    
    ###########
    ...
    bins = 32
    plt.clf()
    plt.hist([scores_good, scores_anomaly], bins, label=['normal', 'abnormal'], color=['green', 'red'])
    ...
    good_index = [index for index, defect_type in enumerate(test_datasets.types) if defect_type == '0_good']
    abnormal_index = [index for index, defect_type in enumerate(test_datasets.types) if defect_type != '0_good']
