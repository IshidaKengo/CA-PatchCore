...
class Segmentation():
    def __init__(self, args):
        ...
        self.dataset_root = f'{args.dataset_path}/'
    ...
    def run(self):
        ...
        dataset_train = MVTecLocoDataset(root_dir=self.dataset_root, category='train/0_good', resize_shape=self.image_size )
        ...
        if StartTrain:
            ...
            torch.save(train_features.cpu(),f'{mid_output_path}/{self.dataset_name}.pth')
            ...
            torch.save(cluster_centers.cpu(),f'{mid_output_path}/{self.dataset_name}_k{num_cluster}.pth')
            ...
            train_features = torch.load(f'{mid_output_path}/{self.dataset_name}.pth').cuda()
            train_features = train_features.cpu().numpy()
            kmeans=KMeans(init='k-means++',n_clusters=num_cluster)
            c = kmeans.fit(train_features)
            cluster_centers = torch.from_numpy(c.cluster_centers_)
        ...
        ##SAVE SegIMAGES##
        savepath = os.path.join('./segimgs', self.dataset_name, f'num_clusters={num_cluster}')

        #train_savepath = f'{savepath}/train'
        train_savepath = os.path.join(savepath, 'train', '0_good')
