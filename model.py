...
class MODEL(pl.LightningModule):
    def __init__(self, args, save_dir):
        ...
        self.segimg_path = os.path.join('./segimgs', args.dataset_name, f'num_clusters={args.num_cluster}')
    ...
    def train_dataloader(self):
        image_datasets = LoadDatasetWithSeg(root=self.args.dataset_path, segimg_path=self.segimg_path, transform=self.data_transforms, phase='train')
        train_loader = DataLoader(image_datasets, batch_size=1, shuffle=False, num_workers=4) #, pin_memory=True)
        return train_loader

    def test_dataloader(self):
        test_datasets = LoadDatasetWithSeg(root=self.args.dataset_path, segimg_path=self.segimg_path, transform=self.data_transforms, phase='test')
    ...
    def on_train_start(self):      
        ...
        self.embedding_dir_path, self.result_path= prep_dirs(self.save_dir)
        self.embedding_list = []
